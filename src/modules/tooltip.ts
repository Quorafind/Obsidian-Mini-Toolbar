// From https://github.com/codemirror/tooltip
          ...
          ? new IntersectionObserver(
              (entries) => {
                if (
                  ...
                  this.intersectionTimeout = window.setTimeout(() => {
    ...
    writeMeasure(measured: Measured) {
      ...
      for (let i = 0; i < this.manager.tooltips.length; i++) {
        ...
        if (this.position == "absolute") {
          ...
          dom.style.top = top - measured.parent.top + 17 * 2 + "px";
          dom.style.left = left - measured.parent.left + "px";
        }
        if (arrow)
          arrow.style.left = `${
...
export function hoverTooltip(
  ...
  return [
    hoverState,
    ViewPlugin.define(
      (view) => new HoverPlugin(view, source, hoverState, setHover, hoverTime),
    ),
